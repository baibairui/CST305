...
void updateLighting() {
    ...
    // 更新光源位置
    float scale_z = 0.6f;
    float lamp_z = -5.0f + base_height + hemisphere_radius * scale_z; // 调整光源位置
...
void drawLampCover() {
    ...
    float scale_z = 0.6f;
    float lamp_z = -4.5f + base_height; // 从地面（-5）开始，加上底座高度
...
void drawLightSource() {
    ...
    glutSolidSphere(0.3f, 20, 20);  // 墛大中等光晕
...
void drawGround() {
    ...
    glColor3f(0.5f, 0.5f, 0.5f); // 灰色墙壁
    
    // 绘制左墙 (在 x = -5 平面)
    glBegin(GL_QUADS);
    glNormal3f(1.0f, 0.0f, 0.0f); // 法向量朝右
    glVertex3f(-5.0f, -5.0f, -5.0f);
    glVertex3f(-5.0f, 5.0f, -5.0f);
    glVertex3f(-5.0f, 5.0f, 5.0f);
    glVertex3f(-5.0f, -5.0f, 5.0f);
    ...
    // 绘制后墙 (在 y = -5 平面)
    glBegin(GL_QUADS);
    glNormal3f(0.0f, 1.0f, 0.0f); // 法向量朝前
    glVertex3f(-5.0f, -5.0f, -5.0f);
    glVertex3f(5.0f, -5.0f, -5.0f);
    glVertex3f(5.0f, -5.0f, 5.0f);
    glVertex3f(-5.0f, -5.0f, 5.0f);
...
const float PARTICLE_SPEED = 0.0008f; // 粒子移动速度
...
bool isParticleIntersectingWalls(const Particle& p) {
    ...
    // 检测与左墙面的相交 (x = -5.0)
    if (abs(p.x + 5.0f) < WALL_THRESHOLD &&
        p.y >= -5.0f && p.y <= 5.0f &&
        p.z >= -5.0f && p.z <= 5.0f) {
        return true;
    }
    ...
    // 检测与后墙面的相交 (y = -5.0)
    if (abs(p.y + 5.0f) < WALL_THRESHOLD &&
        p.x >= -5.0f && p.x <= 5.0f &&
        p.z >= -5.0f && p.z <= 5.0f) {
        return true;
    }
    ...
    // 检测与地面的相交 (z = -5.0)
    if (abs(p.z + 5.0f) < WALL_THRESHOLD &&
        p.x >= -5.0f && p.x <= 5.0f &&
        p.y >= -5.0f && p.y <= 5.0f) {
        return true;
    }
...
void initParticle(Particle& p) {
    ...
    p.x = lamp_x;
    p.y = lamp_y;
    p.z = -5.0f + base_height + hemisphere_radius * 0.6f; // 调整粒子起始高度
